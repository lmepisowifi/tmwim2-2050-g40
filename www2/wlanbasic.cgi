...
band_keys() {
    case "$1" in
        5)
            ...
            RV_PFX="/tmp/wbasic5"
            SEC_PFX="/tmp/wsec5"
            RP_EN_KEY="REPEATER_ENABLED1"
            RP_SSID_KEY="REPEATER_SSID1"
            ;;
        *)
            ...
            RV_PFX="/tmp/wbasic24"
            SEC_PFX="/tmp/wsec24"
            RP_EN_KEY="REPEATER_ENABLED2"
            RP_SSID_KEY="REPEATER_SSID2"
            ;;
    esac
}
...
# Detect a "live-updatable-only" change: every field except SSID/
    ...
    if [ "$ACTION" = "save_iface" ]; then
        ...
        # ── VXD (idx 5) also drives the standalone REPEATER_ENABLED*/
        #    REPEATER_SSID* keys the vendor repeater/extender feature reads,
        #    separate from the MBSSIB table entry. Keep both in lock-step:
        #    REPEATER_ENABLED is the inverse of wlanDisabled (0=off/1=on).
        if [ "$IDX" = "5" ]; then
            RP_EN=1; [ "$FORM_DIS" = "1" ] && RP_EN=0
            mib set "$RP_EN_KEY" "$RP_EN"
            [ -n "$FORM_SSID" ] && mib set "$RP_SSID_KEY" "$FORM_SSID"
            mib commit
            dbg "save_iface idx=5: synced $RP_EN_KEY=$RP_EN ${FORM_SSID:+$RP_SSID_KEY=$FORM_SSID}"
        fi

        # Same sync for a merged-SSID partner, if it's also a VXD slot.
        if [ -n "$PART_PFX" ] && [ "$PART_IDX" = "5" ]; then
            case "$PART_BAND" in
                5) PART_RP_EN_KEY="REPEATER_ENABLED1"; PART_RP_SSID_KEY="REPEATER_SSID1" ;;
                *) PART_RP_EN_KEY="REPEATER_ENABLED2"; PART_RP_SSID_KEY="REPEATER_SSID2" ;;
            esac
            PART_RP_EN=1; [ "$FORM_DIS" = "1" ] && PART_RP_EN=0
            mib set "$PART_RP_EN_KEY" "$PART_RP_EN"
            [ -n "$FORM_SSID" ] && mib set "$PART_RP_SSID_KEY" "$FORM_SSID"
            mib commit
            dbg "save_iface idx=$IDX: merged partner also VXD -> synced $PART_RP_EN_KEY=$PART_RP_EN"
        fi
    ...
    if [ "$ACTION" = "sta_connect" ]; then
        ...
        # REPEATER_SSID* tracks the VXD's target network too, not just the
        # MBSSIB table entry — otherwise it goes stale whenever Site Survey's
        # Connect button is used instead of the Basic Settings save form.
        if [ "$IDX" = "5" ]; then
            mib set "$RP_SSID_KEY" "$FORM_SSID"
            mib commit
            dbg "sta_connect idx=5: synced $RP_SSID_KEY=$FORM_SSID"
        fi
